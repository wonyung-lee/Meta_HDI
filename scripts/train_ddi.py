...
def main():
    ...
    out_dir = Path(cfg.get("out_dir", "outputs/ddi"))
    ...
    train_loader = DataLoader(train_ds, batch_size=int(cfg["train"].get("batch_size", 64)),
                              shuffle=True, num_workers=0, collate_fn=collate, pin_memory=("cuda" in device_str))
    val_loader   = DataLoader(val_ds, batch_size=int(cfg["train"].get("batch_size", 64)),
    ...
    if init_cfg.get("embedding"):
        sd = torch.load(init_cfg["embedding"], map_location="cpu")
        model.load_state_dict(sd, strict=False)
        print(f"[info] Loaded init.embedding: {init_cfg['embedding']} (strict=False)")
    if init_cfg.get("predictor"):
        sd = torch.load(init_cfg["predictor"], map_location="cpu")
        model.load_state_dict(sd, strict=False)
        print(f"[info] Loaded init.predictor: {init_cfg['predictor']} (strict=False)")

    # ----- Optimizer
    lr = float(cfg["train"].get("lr", 7e-4))
    ...
    for ep in range(1, epochs+1):
        ...
        if val["auroc_macro"] > best["auroc_macro"]:
            best = {"epoch": ep, **val}
            torch.save(model.state_dict(), out_dir / "ddi_pretrained.pt")
            print(f"[info] Saved best to {out_dir / 'ddi_pretrained.pt'}")
